...
from agent.helpers import _is_navigation_back_goal, _loose_contains, _safe_asdict

class Validator:
    """独立规则验证器：不调用 Planner 的 LLM。"""
    ...
    def validate(
        ...
        if decision.type == DecisionType.COMPLETE:
            return self.validate_goal(after, task_goal, progress=progress, memory=memory)
        ...
        if action.type in {ActionType.CLICK, ActionType.SWIPE_UP, ActionType.SWIPE_DOWN, ActionType.BACK, ActionType.HOME}:
            ...
            return ValidationResult(
                True if changed or action.type in {ActionType.BACK, ActionType.HOME} else True,
        ...
        if action.type == ActionType.INPUT:
            ...
            # 部分 App 不会把输入内容回写到 source，动作成功时不强制失败。
    ...
    def validate_goal(
        ...
        if progress and memory and progress.completed_objectives and memory.action_history:
            ...
            if progress_matched and successful_actions and len(progress_matched) == len(criteria):
                return ValidationResult(
                    True,
